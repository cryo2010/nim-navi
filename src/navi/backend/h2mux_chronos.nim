## HTTP/2 connection multiplexer, chronos backend. The whole mux body is the shared
## `h2mux_common.nim` include; only the chronos-specific pieces below (fire-and-forget
## send, keepAlive timer, the cancellation-aware reader teardown, `newH2Mux`, `close`)
## are written per backend. See `h2mux_common.nim` for the design.
...
proc fireSend(mux: H2Mux, data: string) =
  ## chronos: `asyncSpawn` the error-swallowing `trySend` (raw-spawning `send` would
  ## leave an unhandled exception if the transport is gone).
  asyncSpawn mux.trySend(data)
  ...
  try:
    while mux.alive:
      ...
      if interval <= 0:
        ...
        # connection closes rather than busy-spinning on a zero-length timer. A later
        # re-enable spawns a fresh loop via `applyKeepAlive`, so this one can exit.
        mux.keepAliveRunning = false
        break
      let timer = sleepAsync(interval.milliseconds)
...
proc newH2Mux*(transport: be.Conn, maxBody = 0, decompress = false,
  ...
  if keepAliveMs > 0:
    mux.keepAliveRunning = true
    asyncSpawn keepAlive(mux)   # self-exits when readerDone completes
  result = mux
