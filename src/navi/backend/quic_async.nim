...
proc requestOnConn*(qc: QuicConnAsync, verb, path: string,
  ...
  ## `trailers` are sent after the body as a trailing HEADERS section.
  if not qc.alive:
...
proc streamTrailers*(qc: QuicConnAsync, sid: int64): seq[(string, string)] =
...
proc openConnect*(qc: QuicConnAsync, path: string, headers: seq[(string, string)],
...
proc tunnelSend*(qc: QuicConnAsync, sid: int64, data: string): Future[void] {.async.} =
...
proc tunnelRecv*(qc: QuicConnAsync, sid: int64): Future[string] =
  ## The next inbound tunnel chunk, or "" once the peer half-closes.
  qc.readStreamBody(sid)

proc tunnelClose*(qc: QuicConnAsync, sid: int64) {.async.} =
