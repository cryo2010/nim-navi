## End-to-end transparent HTTP/3 dispatch on the chronos backend: the chronos
## client talks to Caddy over h2 first (learning Alt-Svc), then transparently
## upgrades subsequent buffered requests to HTTP/3, driven by the chronos event
## loop. The chronos twin of dispatch_async_test.nim. Built with -d:ssl -d:naviHttp3.
...
proc main() {.async.} =
  ...
  let r3 = await api.post("https://localhost:4433/echo", body = "chronos-body-7")
  doAssert r3.status == 200 and r3.httpVersion == "HTTP/3"
  doAssert r3.body == "echo:chronos-body-7", "POST echo mismatch: " & r3.body
  echo "POST over ", r3.httpVersion, " echoed the uploaded body"

  await api.close()
  echo "NAVI HTTP/3 CHRONOS DISPATCH OK"
