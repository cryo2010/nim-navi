...
## using checkmate's time travel: inside a `controlTime` block the async `sleep`
## is instant but the virtual clock advances, so we can assert real pacing and
## in-flight caps in milliseconds without wall-clock flakiness or slow tests.
##
...
## a plain `nimble test` (no checkmate overlay) a `controlTime` block runs with
## real clocks and `timeTravelActive()` is false, so the tests skip themselves;
## the checkmate CI step runs them for real. Run with: `checkmate tt_mw_timing`.

import std/[unittest, times]
import navi/asyncdispatch
import navi/asyncdispatch/mw
import checkmate                     # controlTime / timeTravelActive
...
proc workSim(peak, cur: ref int, workMs: int): NaviMiddleware =
  result = proc(ctx: NaviContext): Future[void] {.async.} =
    ...
    await sleep(workMs)              # virtual inside a controlTime block
    dec cur[]
    await ctx.next()
  ...
  test "five requests at 10/s (burst 1) span ~400ms of virtual time":
    controlTime:
      if not timeTravelActive():
        skip()                      # plain nimble test: no overlay, real clocks
      else:
